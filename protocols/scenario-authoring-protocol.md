...
### 5.5 Systems-Math Linking Rule

모든 Scenario는 가능한 경우 관련 Systems-Math 문서를 연결해야 한다.
...
### 5.6 Experiment Linking Rule

Scenario는 관련 Experiment 문서를 연결할 수 있다.
...
### 4.13 Evidence Correlation Rule

Scenario는 metrics, logs, traces 간 correlation을 설명해야 한다.

**예:**

```
- Kafka lag 증가
- consumer rebalance log 발생
- trace latency 증가
```
...
### 4.14 Degraded Observability Rule

partial observability, missing metrics, trace sampling loss 상황에서는:

Scenario는 **certainty degradation 가능성**을 설명해야 한다.

Unknown을 추정으로 대체해서는 안 된다.

---

### 4.15 Recovery Signal Rule

Scenario는 정상 복구 판단 기준을 정의해야 한다.
...
### 4.16 FinTech Consistency Rule

결제 시스템 Scenario는 반드시 다음 위험을 고려해야 한다.
